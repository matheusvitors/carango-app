import { HttpError } from "@/application/interfaces";
import { AxiosError } from "axios";

export const httpErrorHandler = (error: any, from: string): HttpError => {

	const errors: any = {
		0: { status: 0, message: error.message},
		401: { status: 401, message: 'Usuário não autorizado.'},
		404: { status: 404, message: 'Recurso não encontrado'},
		405: { status: 405, message: 'Método HTTP não suportado pelo servidor.'},
		422: { status: 422, message: error.message},
